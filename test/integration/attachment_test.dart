...
void main() {
  ...
  group('Attachment Test', () {
    ...
    test('Upload an attachment', () async {
      final response = await clickUp.attachments.createTaskAttachment(
          filePath:
              p.join(p.current, "test", "integration", "beksinski_sample.jpg"),
          taskID: "213123",
          useCustomTaskID: false);
      expect(response, {
        'message':
            'Mock server error. Media type multipart/form-data deserialization is not supported.'
      });
    });
  });
}
