...
class ClickUpAuth {
  ...
  ClickUpAuth({required this.endPoint, required this.authToken});
  ...
  Future<Map<String, dynamic>> getAuthorizedUser() async {
    try {
      final response = await get(Uri.parse("$endPoint/user"),
  ...
  Future<Map<String, dynamic>> getAuthorizedTeams() async {
    try {
      final response = await get(Uri.parse("$endPoint/team"),
          headers: {"Authorization": authToken});
